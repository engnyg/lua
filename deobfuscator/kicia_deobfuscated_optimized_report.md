...
共改名 64920 個反編譯器產生的名稱（`vN` / `tN` / `fN`），完整對照在 JSON 報告的 `renames`。改名後重新解析整份檔案，每個變數引用指向的宣告都和改名前相同。
...
| manual | `names.json` 人工命名 | `loadEquipCooldownModifier` | 25173 |
| manual (upvalue) | `names.json` 人工命名的上值 | `up0` → `settings` | 156 |
